...
def schedule_all_get():
    ...
    with db_session.create_session() as session:
        ...
        return flask.jsonify(
            {'schedule': [
                item.to_dict(
                    only=('class_name', 'day_of_week', 'subject.subject_name', 'classroom_id', 'number_lesson'))
                for item
                in
                schedule]})
...
def schedule_get(grade_level):
    ...
    with db_session.create_session() as session:
        classes_ids = [classes.id for classes in
                       session.query(Classes).filter(Classes.grade_level == grade_level).all()]
